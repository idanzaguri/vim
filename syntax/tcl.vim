...
syn keyword tclStatement	proc global return lindex exec append split join
syn keyword tclStatement	llength lappend lreplace lrange list concat incr lsearch regexp regsub
syn keyword tclStatement	upvar set puts unset close open file
...
syn keyword tcltkStringSwitch	contained	compare first index last length match range tolower toupper trim trimleft trimright wordstart wordend equal
...
syn keyword tcltkArraySwitch	contained	anymore donesearch exists get names nextelement size startsearch set unset
...
"Changed by eranbu - the '\' for line continue must be the last char (no whitespace)
...
syn match  rabbit		"Rbt\w*"
syn match  dapne		"Dph\w*"
syn match  viper		"Vpr\w*"
syn match  flag                 "-\w*"
...
if version >= 508 || !exists("did_tcl_syntax_inits")
  ...
  HiLink rabbit                Statement
  HiLink dapne                 Statement
  HiLink flag                  Constant
